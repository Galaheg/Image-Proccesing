...
tamsayi_degisken = 10#int vs yazmaya gerek yokmuş
ondalikli_sayi = 12.3
...
karakter = 'c'
print("Karatker: {} ".format(karakter))
...
# anahtar kelime
de = 4    
# def = 4 definition kısaltması def 
...
print(hafta[-1])#liste sonuna gider
...
print("Appended 7 : {}".format(sayi_listesi))
...
#tuple_veritipi.append(7) --> cannot change tupples

# 2. indeksten sonraki elemanları yazdır
print(tuple_veritipi[2:])
print(tuple_veritipi[2:5])
 # count eleman 
print(tuple_veritipi.count(3))#kaç tane 3 var onu sayar
...
dq.appendleft(3) # 3 ekle soldan [3,1,2]
print(dq)
...
- { "anahtar": değer }
- javadaki map yapısına benziyor
"""
...
if deger in liste:#for döngüsüne gerek kalmadan if içinde kontrol edebildik
...
# for 
for i in range(1,11):#kendi içinde for kullanıyor
    print(i)
...
def katsayiCarpimi():
    global katsayi#publici içerden yapıoruz
...
# boş fonk.
def bos():#boş fonksyion kullanmak istersek pass kullancaz
     pass
    
# built in fonk.
liste = [1,2,3,4]
print(len(liste))
print(str(liste))#stringe çevir lsiteyi
liste2 = liste.copy()
print(liste2)
print(max(liste2))
print(min(liste))
...
generator = (x for x in range(1,4))#birer birer generate edildiği için bellekte yer tutamaz
for i in generator:
    print(i)
...
dizi2 = dizi.reshape(3, 5)#iki boyutlu dizi oluşturduk bir bouyutlu diziden yararlanarak
...
# arange(x,y,basamak)
dizi_aralik = np.arange(10,50,5)#increase 5 between ranges
print(dizi_aralik)

# linspace(x,y, basamak) 
dizi_bosluk = np.linspace(10,20,5)#split array between ranges
print("Bosluklu:{}".format(dizi_bosluk))
...
plt.xlabel("x")
plt.ylabel("y")
plt.grid(True)#grid lines shows up
plt.xticks([0,1,2,3,4,5])
plt.legend()#labelların görünmesi için bu fosnkiyon lazımmış
plt.show()

plt.figure()
plt.plot(x,y,color="purple",alpha=0.7,label="line")
plt.scatter(x,y,color="yellow",alpha=0.4,label="scatterlo")
plt.title("denemem")
plt.xlabel("x")
plt.ylabel("y")
plt.grid(True)
...
print(os.name)#nt eindows demek
currentDirectory = os.getcwd()
print(currentDirectory)
folder_name="zaafolder"
os.mkdir(folder_name)
new="haha"
os.rename(folder_name,new)

os.chdir(currentDirectory+"\\"+new)
print(os.getcwd())
...
for i in os.walk(currentDirectory):
    print(i)
